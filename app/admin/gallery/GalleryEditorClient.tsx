...
type SettingsDraft = Omit<GallerySettings, 'id' | 'updatedAt'>;
...
type DraftItem = Omit<GalleryItem, 'order' | 'createdAt'> & {
  id: string;
  order: number;
};

const DRAFT_KEY = 'admin.gallery.draft';
...
export function GalleryEditorClient({ initial }: { initial: InitialPage }) {
  const { user } = useAuth();
    ...
    async (pageToLoad: number) => {
      ...
      try {
        const res = await fetch(
          `/api/gallery?page=${pageToLoad}&pageSize=${pageSize}`,
          {
  ...
  const ensureAllPagesLoaded = useCallback(async () => {
    const res = await fetch(
      `/api/gallery?page=1&pageSize=${pageSize}`,
      { headers: { 'x-auth-email': user?.email ?? '' } },
    );
  ...
  const galleryColumns = useMemo(() => [
    ...
    },
    {
      ...
      className: 'w-24',
      render: (item: DraftItem) => (
        <label className="inline-flex items-center gap-2 self-start text-xs text-dark-gray">
          <input
            type="checkbox"
            checked={Boolean(item.large)}
            onChange={(e) => updateItem(item.id, { large: e.target.checked })}
            className="h-4 w-4 rounded border-light-gray text-hunter-green focus:ring-hunter-green"
          />
          <span className="font-semibold uppercase tracking-wider">Featured</span>
        </label>
      ),
    },
  ...
  const onSaveAll = async () => {
    ...
    try {
      ...
      const savable = items.filter(
        (i) => i.src.trim().length > 0 && i.alt.trim().length > 0,
      );
      ...
      const updatedItems = savable.filter((i) => {
        ...
        return (
          before.src !== i.src ||
          before.alt !== i.alt ||
          Boolean(before.large) !== Boolean(i.large)
        );
      });
      ...
      for (const i of newItems) {
        const result = await apiFetch('item', {
          item: { src: i.src.trim(), alt: i.alt.trim(), large: Boolean(i.large) },
        });
        const persisted = (result as { item?: GalleryItem }).item;
        if (persisted?.id) {
          setItems((prev) =>
  ...
  return (
    <div className="container-base section-padding">
      <header className="admin-header">
        <div>
          ...
          <p className="mt-1 text-sm text-dark-gray">
            Edit section Gallery dan kartu foto yang tampil di home page
          </p>
        </div>
      ...
      <section className="mb-8 rounded-2xl border border-light-gray bg-white p-6">
        ...
        <p className="mt-1 text-sm text-dark-gray">
          Tag, judul, dan subtitle section Gallery
        </p>
              ...
              <>
                <div className="admin-card-header">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-medium text-hunter-green truncate">{item.alt || 'Foto belum diisi'}</h3>
                ...
                <div className="admin-card-body">
                  <div className="admin-card-row">
                    ...
                    <span className="admin-card-value flex-1 truncate text-xs text-dark-gray">{item.src || '—'}</span>
                  </div>
                  <div className="admin-card-row">
                  ...
                  <div className="admin-card-row">
                    <span className="admin-card-label">Featured:</span>
                    <span className="admin-card-value flex-1 truncate">
                      {Boolean(item.large) ? (
                        ...
                        <span className="rounded-full bg-light-gray px-2 py-0.5 text-[0.65rem] font-semibold uppercase tracking-wider text-dark-gray">
                          Tidak
                        </span>
                      )}
                    </span>
