...
type DraftItem = Omit<FAQ, 'order' | 'createdAt'> & {
  id: string;
  order: number;
};

const DRAFT_KEY = 'admin.faq.draft';
...
export function FAQEditorClient({ initial }: { initial: InitialPage }) {
  const { user } = useAuth();
    ...
    async (pageToLoad: number) => {
      ...
      try {
        const res = await fetch(
          `/api/faq?page=${pageToLoad}&pageSize=${pageSize}`,
          {
  ...
  const ensureAllPagesLoaded = useCallback(async () => {
    const res = await fetch(
      `/api/faq?page=1&pageSize=${pageSize}`,
      { headers: { 'x-auth-email': user?.email ?? '' } },
    );
  ...
  const onSaveAll = async () => {
    ...
    try {
      ...
      const savable = items.filter((i) => i.question.trim().length > 0);
      ...
      const updatedItems = savable.filter((i) => {
        ...
        return (
          before.question !== i.question || before.answer !== i.answer
        );
      });
      ...
      for (const i of newItems) {
        ...
        const persisted = (result as { item?: FAQ }).item;
        if (persisted?.id) {
          setItems((prev) =>
      ...
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-dark-gray">
            Edit section FAQ dan daftar pertanyaan yang tampil di home page
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex flex-wrap gap-x-3 gap-y-1">
            ...
            <Link href="/admin/invite" className={NAV_LINK_CLS}>
              Invite Members
            </Link>
          </div>
