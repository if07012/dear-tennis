// ============================================
// /api/faq — FAQ content read + admin write
// ============================================
//
// GET  → public. Accepts `?page=&pageSize=` only when `x-auth-email` matches
//        ADMIN_EMAIL; otherwise returns the full FAQ list.
...
//   { kind: 'item',    item:  { id?, question, answer } }
...
export async function GET(request: Request) {
  ...
  if (wantsPagination) {
    ...
    const content = await getFAQContentForAdmin({ page, pageSize });
...
type SettingsBody = {
  kind: 'settings';
  settings: Omit<FAQSettings, 'id' | 'updatedAt'> & { updatedAt?: string };
};
...
export async function PUT(request: Request) {
  ...
  try {
    await ensureFAQSheets(spreadsheetId);

    switch (body.kind) {
      case 'settings': {
        const next: FAQSettings = {
          id: SETTINGS_ROW_ID,
        ...
        const existingId = await getFAQSettingsRowId(spreadsheetId);
        if (existingId) {
          await updateRowById(
            spreadsheetId,
            SETTINGS_SHEET,
      ...
      case 'item': {
        ...
        if (question.length === 0) return badRequest('question is required');
        const answer = String(item.answer ?? '').trim();
        const clean = { question, answer };

        if (item.id) {
          // An item id may refer to a static fallback (e.g. 'beginner') that
          ...
          if (existingRow) {
            ...
            const updated: FAQ = { id: item.id, ...clean, order };
            clearFAQContentCache();
            return NextResponse.json({ ok: true, item: updated });
          }
          const order = await getFAQItemCount(spreadsheetId);
          ...
          const created: FAQ = { id: item.id, ...clean, order };
          clearFAQContentCache();
        ...
        const created: FAQ = { id: newId, ...clean, order };
        clearFAQContentCache();
        return NextResponse.json({ ok: true, item: created });
      }
    ...
    return serverError('Failed to save FAQ content');
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    console.error('Error in DELETE /api/faq:', error);
